...
async function loadCatalog(){{
  try{{
    ...
    data.modules.forEach(m => {{
      ...
      li.innerHTML = `<b>${{m.name}}</b> [${{m.framework}}] — ${{m.commands.join(', ')}}`;
      ul.appendChild(li);
    }});
  }}catch(e){{
...
"""
    ...
    async def module_catalog(self, _: web.Request) -> web.Response:
        ...
        for cm in class_module_manager.catalog():
            data.append(
                {
                    "name": cm["name"],
...
class_module_manager = ModuleManager()
...
async def process_builtin(client: MaxClient, packet: dict, chat_id: int, message_id: int, cmd: str, arg: str) -> bool:
    ...
    # New class-based module system dispatch
    module_response = await class_module_manager.dispatch(
        command_name=cmd,
        message_ctx={"chat_id": destination_chat, "message_id": message_id, "packet": packet},
        args_raw=arg,
    )
    if module_response is not None:
        await edit_message(client, destination_chat, message_id, module_response)
        return True
...
async def main() -> None:
    ...
    # class-based module framework (MaxUB 2.0 style)
    class_module_manager.discover_from_path(CORE_MODULES_DIR)
    class_module_manager.discover_from_path(MODULES_DIR)
