...
- 🧩 **Гибкая система модулей:** legacy + class-based (`core.loader`, `@loader.tds`, `.loadmod`, `.dlm <url>`)
- 💬 **Поддержка чатов:** работа в личках и чатах
- 🌐 **Web UI:** просмотр модулей, редактирование конфигов, аккаунты, каталог модулей и стрим логов
- ⚙️ **Конфигурация:** `.config`, `.fconfig`, хранение в `userbot_config.json`
- 👥 **Мультиаккаунт база:** `accounts.json` + UI-форма добавления
- 😀 **API-функции:** реакции, смена профиля, работа с избранным
...
См. [`MODULES.md`](MODULES.md), [`mods/modules.md`](mods/modules.md) и примеры в `modules/examples/`.
